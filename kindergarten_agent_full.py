...
class DailyActivity(BaseModel):
    """每日活动"""
    time: str = Field(description="时间段，如：早晨、下午、傍晚、睡前")
    activity: str = Field(description="活动内容")
    goal: str = Field(description="活动目标")


class TransitionPlan(BaseModel):
    """幼小衔接计划"""
    duration: str = Field(default="3个月", description="计划周期")
    weekly_goals: List[str] = Field(default_factory=list, description="每周重点目标")
    daily_activities: List[DailyActivity] = Field(default_factory=list, description="每日推荐活动")
    resources: List[str] = Field(default_factory=list, description="推荐资源")
    parent_tips: List[str] = Field(default_factory=list, description="家长注意事项")
...
{knowledge_base}"""
    ...
    def generate_plan(self, profile: ChildProfile, duration: str = "3个月") -> dict:
        """生成个性化计划 - 使用 structured output 确保 JSON 格式"""
...
- 家长担忧：{', '.join(profile.concerns) if profile.concerns else '暂无'}
...
"""
        ...
        try:
            # 使用 with_structured_output 确保 JSON 格式输出
            structured_llm = self.llm.with_structured_output(TransitionPlan)
            plan = structured_llm.invoke(prompt)
            return plan.model_dump()
        except Exception as e:
            logger.warning(f"Structured output 失败，尝试 fallback: {e}")
            # Fallback: 使用原有 JSON 解析方式
            return self._generate_plan_fallback(profile, assessment, duration)

    def _generate_plan_fallback(self, profile: ChildProfile, assessment: AssessmentResult, duration: str) -> dict:
        """生成计划的备用方法"""
        prompt = f"""请为以下孩子生成一个{duration}的幼小衔接计划：
...
- 年龄：{profile.age}岁
- 整体水平：{assessment.overall_level}
- 优势：{', '.join(assessment.strengths) if assessment.strengths else '暂无'}
- 需加强：{', '.join(assessment.areas_to_improve) if assessment.areas_to_improve else '暂无'}

请严格只返回JSON格式，不要包含解释或markdown代码块：
{{"duration": "3个月", "weekly_goals": ["目标1", "目标2"], "daily_activities": [{{"time": "早晨", "activity": "活动", "goal": "目标"}}], "resources": ["资源1"], "parent_tips": ["建议1"], "evaluation_criteria": ["标准1"]}}
"""
        ...
        if isinstance(raw_content, list):
            ...
            if text_chunks:
                ...
                return self._default_plan(duration)
    ...
    def _default_plan(self, duration: str = "3个月") -> dict:
        """返回默认计划"""
        return {
            "duration": duration,
            "weekly_goals": [
