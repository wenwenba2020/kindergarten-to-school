...
def calculate_assessment(profile: Dict) -> Dict:
    ...
    # 新增维度
    focus = _score(profile.get("focus", 3))
    emotion = _score(profile.get("emotion", 3))
    time_awareness = _score(profile.get("time_awareness", 3))

    # 计算总分（扩展为8个维度）
    total = (lang_avg + math_avg + social + self_care + motor +
             focus + emotion + time_awareness)
